...
<?php $component = App\View\Components\AppLayout::resolve(['title' => 'Admin - Kelola Reschedule Murid'] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $component->withAttributes([]); ?>
     <?php $__env->slot('header', null, []); ?> 
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            <?php echo e(__('Kelola Reschedule Murid')); ?>
    ...
    <div class="py-12" x-data="{ 
        ...
        notes: '',
        allSchedules: <?php echo e($availableSchedules->toJson()); ?>,
            ...
            <div class="bg-white p-6 rounded-lg shadow sm:rounded-lg">
                ...
                <div class="flex flex-wrap items-center justify-between border-b border-gray-200 mb-6 gap-4 pb-4">
                    ...
                    <div class="flex space-x-2">
                        <a href="<?php echo e(route('admin.reschedule.index', ['status' => 'pending', 'category' => $category])); ?>" 
                           class="py-2 px-4 rounded-lg font-semibold text-xs transition flex items-center gap-2 <?php echo e($status === 'pending' ? 'bg-[#101828] text-[#D3AF37]' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'); ?>">
                            <i class="fa-solid fa-clock-rotate-left"></i>
                            Menunggu Reschedule
                            <span class="px-2 py-0.5 text-[10px] rounded-full bg-amber-400 text-[#101828] font-bold">
                                <?php echo e(\App\Models\RescheduleQueue::where('status', 'pending')->count()); ?>

                            </span>
                        </a>
                        <a href="<?php echo e(route('admin.reschedule.index', ['status' => 'rescheduled', 'category' => $category])); ?>" 
                           class="py-2 px-4 rounded-lg font-semibold text-xs transition flex items-center gap-2 <?php echo e($status === 'rescheduled' ? 'bg-[#101828] text-[#D3AF37]' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'); ?>">
                            <i class="fa-solid fa-circle-check"></i>
                            Sudah Di-reschedule
                            <span class="px-2 py-0.5 text-[10px] rounded-full bg-green-500 text-white font-bold">
                                <?php echo e(\App\Models\RescheduleQueue::where('status', 'rescheduled')->count()); ?>

                            </span>
                        </a>
                        <a href="<?php echo e(route('admin.reschedule.index', ['status' => 'all', 'category' => $category])); ?>" 
                           class="py-2 px-4 rounded-lg font-semibold text-xs transition flex items-center gap-2 <?php echo e($status === 'all' ? 'bg-[#101828] text-[#D3AF37]' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'); ?>">
                            Semua Status
                        </a>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <a href="<?php echo e(route('admin.reschedule.index', ['status' => $status, 'category' => 'all'])); ?>" 
                           class="px-3 py-1 text-xs rounded-full border <?php echo e($category === 'all' ? 'bg-[#D3AF37] text-[#101828] font-extrabold border-[#D3AF37]' : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-50'); ?>">
                            Semua
                        </a>
                        <a href="<?php echo e(route('admin.reschedule.index', ['status' => $status, 'category' => 'belajar'])); ?>" 
                           class="px-3 py-1 text-xs rounded-full border <?php echo e($category === 'belajar' ? 'bg-[#D3AF37] text-[#101828] font-extrabold border-[#D3AF37]' : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-50'); ?>">
                            Belajar
                        </a>
                        <a href="<?php echo e(route('admin.reschedule.index', ['status' => $status, 'category' => 'prestasi'])); ?>" 
                           class="px-3 py-1 text-xs rounded-full border <?php echo e($category === 'prestasi' ? 'bg-[#D3AF37] text-[#101828] font-extrabold border-[#D3AF37]' : 'bg-white text-gray-600 border-gray-300 hover:bg-gray-50'); ?>">
                            Prestasi
                        </a>
                    </div>
                </div>
                ...
                <div class="relative overflow-x-auto border sm:rounded-lg">
                    <table class="w-full text-sm text-left text-gray-500">
                        ...
                        <tbody>
                            <?php $__empty_1 = true; $__currentLoopData = $queues; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                <tr class="bg-white border-b hover:bg-gray-50">
                                    <td class="px-4 py-4 text-center font-medium text-gray-900">
                                        <?php echo e(($queues->currentPage() - 1) * $queues->perPage() + $index + 1); ?>

                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="font-bold text-gray-800"><?php echo e($item->student->name); ?></div>
                                        <div class="text-xs text-gray-400">Ortu: <?php echo e($item->student->user->name ?? '-'); ?></div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="font-bold text-slate-800"><?php echo e($item->swimmingClass->name ?? '-'); ?></div>
                                        <?php
                                            $catSlug = strtolower($item->swimmingClass->category->slug ?? 'belajar');
                                        ?>
                                        <span class="inline-block mt-1 text-[10px] font-extrabold px-2.5 py-0.5 rounded-full <?php echo e($catSlug === 'prestasi' ? 'bg-amber-100 text-amber-800 border border-amber-300' : 'bg-cyan-100 text-cyan-800 border border-cyan-300'); ?>">
                                            <?php echo e(ucfirst($catSlug)); ?>

                                        </span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="font-bold text-amber-700">
                                            <i class="fa-solid fa-calendar-xmark mr-1"></i> <?php echo e($item->original_date->translatedFormat('d F Y')); ?>

                                        </div>
                                        <div class="text-xs text-gray-500 mt-0.5">
                                            Coach Izin: <span class="font-semibold text-gray-700"><?php echo e($item->coachLeave->coach->name ?? '-'); ?></span>
                                        </div>
                                        <div class="text-[11px] text-gray-400">
                                            <?php echo e($item->schedule->day_name ?? ''); ?> (<?php echo e($item->schedule->time_range ?? ''); ?>) @ <?php echo e($item->schedule->location->name ?? ''); ?>

                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        <?php if($item->status === 'rescheduled'): ?>
                                    ...
                                    <td class="px-6 py-4">
                                        <?php if($item->status === 'rescheduled' && $item->rescheduled_date): ?>
                                            <div class="font-bold text-green-700">
                                                <i class="fa-solid fa-calendar-check mr-1"></i> <?php echo e($item->rescheduled_date->translatedFormat('d F Y')); ?>

                                            </div>
                                            <div class="text-xs text-gray-600">
                                                Coach: <?php echo e($item->rescheduledSchedule->coach->name ?? '-'); ?>

                                            </div>
                                            <div class="text-[11px] text-gray-400">
                                                <?php echo e($item->rescheduledSchedule->time_range ?? ''); ?> @ <?php echo e($item->rescheduledSchedule->location->name ?? ''); ?>
                                    ...
                                    <td class="px-4 py-4 text-center">
                                        <button @click="openRescheduleModal(<?php echo e(json_encode($item)); ?>)" 
                                            ...
                                            <?php echo e($item->status === 'rescheduled' ? 'Ubah' : 'Reschedule'); ?>
...
<?php endif; ?>
<?php /**PATH D:\laragon\www\klub-renang\resources\views/admin/reschedule/index.blade.php ENDPATH**/ ?>