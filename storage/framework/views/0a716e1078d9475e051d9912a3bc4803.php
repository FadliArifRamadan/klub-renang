        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div
                ...
                <h1 class="text-[#D3AF37] text-2xl md:text-3xl font-extrabold tracking-tight mb-2 flex items-center gap-2">
                    <span>Dashboard, <?php echo e(Auth::user()->name); ?>!</span>
                    <span class="text-xs bg-[#D3AF37]/20 border border-[#D3AF37]/40 text-[#D3AF37] px-3 py-1 rounded-full uppercase tracking-wider font-extrabold">
                        <?php echo e(Auth::user()->role === 'admin_finance' ? 'Admin Finance' : 'Admin Operasional'); ?>

                    </span>
                </h1>
                <p class="text-slate-300 text-sm max-w-3xl leading-relaxed font-normal">
                    <?php if(Auth::user()->role === 'admin_finance'): ?>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            ...
            <div class="lg:col-span-2 rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6 shadow-theme-sm">
                <div class="flex items-center justify-between border-b border-gray-200 dark:border-gray-800 pb-4 mb-5">
                    ...
                    <a href="<?php echo e(route('admin.payments.index')); ?>" class="text-xs font-extrabold text-[#D3AF37] hover:underline flex items-center gap-1">
                ...
                <?php if($recentPendingPayments->isNotEmpty()): ?>
                    <div class="overflow-x-auto">
                        <table class="w-full text-xs text-left text-slate-600 dark:text-slate-300">
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                                <?php $__currentLoopData = $recentPendingPayments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pay): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                        <td class="px-4 py-3.5">
                                            <p class="font-bold text-slate-800 dark:text-white text-xs"><?php echo e($pay->student->name ?? 'Murid Tidak Ditemukan'); ?></p>
                                            <p class="text-[10px] text-slate-400">Ortu: <?php echo e($pay->student->user->name ?? '-'); ?></p>
                                        </td>
                                        <td class="px-4 py-3.5 font-semibold text-slate-700 dark:text-slate-300">
                                            <?php echo e($pay->student->package->name ?? 'Paket'); ?>
                                        ...
                                        <td class="px-4 py-3.5 text-center">
                                            <a href="<?php echo e(route('admin.payments.index')); ?>" class="px-3 py-1.5 bg-[#D3AF37] hover:bg-[#B89426] text-[#101828] text-[11px] font-extrabold rounded-lg shadow-sm transition-all inline-flex items-center gap-1">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6 shadow-theme-sm flex flex-col justify-between">
                <div>
                    ...
                    <div class="space-y-4">
                        <div class="bg-gradient-to-br from-slate-900 to-slate-800 p-5 rounded-2xl border border-[#D3AF37]/30 text-white shadow-md">
                            ...
                            <h4 class="text-2xl font-black text-[#D3AF37]">Rp <?php echo e(number_format($thisMonthApprovedRevenue, 0, ',', '.')); ?></h4>
                            <p class="text-[10px] text-slate-300 mt-1">Berdasarkan transaksi terverifikasi bulan <?php echo e(now()->translatedFormat('F Y')); ?></p>
                        </div>
                        ...
                        <div class="bg-slate-50 dark:bg-slate-800/40 p-4 rounded-xl border border-slate-200 dark:border-slate-700 flex justify-between items-center">
                            <div>
                                ...
                                <span class="text-base font-extrabold text-slate-800 dark:text-white">Rp <?php echo e(number_format($totalApprovedRevenue, 0, ',', '.')); ?></span>
                            </div>
                        ...
                        <div class="bg-slate-50 dark:bg-slate-800/40 p-4 rounded-xl border border-slate-200 dark:border-slate-700 flex justify-between items-center">
                            <div>
                                ...
                                <span class="text-base font-extrabold text-amber-600"><?php echo e($pendingPaymentCount); ?> Pembayaran</span>
                            </div>
                ...
                <div class="pt-6 border-t border-slate-100 dark:border-slate-800 mt-6">
                    <a href="<?php echo e(route('admin.payments.index')); ?>" class="w-full py-3 bg-[#101828] hover:bg-black text-[#D3AF37] text-xs font-extrabold rounded-xl flex items-center justify-center gap-2 shadow-md transition-all">
    ...
    <?php if(Auth::user()->isAdminOperasional()): ?>
        ...
        <div
            ...
            <div>
                ...
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Pilih murid di samping untuk memantau performa
        ...
        <div id="chart-empty-state" class="flex-1 flex flex-col items-center justify-center text-center py-16 px-4">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-2 max-w-sm">Grafik perkembangan akan memvisualisasikan
        ...
        <div id="chart-no-data-state" class="hidden flex-1 flex-col items-center justify-center text-center py-16 px-4">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-2 max-w-sm" id="no-data-subtext">Hubungi Coach
        ...
        <div id="chart-year-empty-state"
            ...
            <p class="font-medium text-gray-800 dark:text-white/90 text-lg" id="year-empty-title">Belum ada data latihan
                di tahun ini</p>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-2 max-w-sm" id="year-empty-subtext">Pilih tahun lain
        ...
        <div id="chart-container" class="hidden flex-1 flex-col">
            ...
            <div id="prestasi-charts-container" class="hidden flex-col space-y-6 w-full mt-2">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div
                        ...
                        <h4 class="text-sm font-bold text-center text-gray-800 dark:text-white/90 mb-4">Kondisi Fisik
                        </h4>
                    ...
                    <div
                        class="bg-gray-50 dark:bg-gray-800/50 p-5 rounded-xl border border-gray-200 dark:border-gray-700">
                        <h4 class="text-sm font-bold text-center text-gray-800 dark:text-white/90 mb-4">Sistem Energi
                        </h4>
                        <div class="relative w-full h-[250px]">
                ...
                <div class="bg-gray-50 dark:bg-gray-800/50 p-5 rounded-xl border border-gray-200 dark:border-gray-700">
                    <div class="flex flex-col sm:flex-row justify-between items-center mb-4 gap-2">
                        <h4 class="text-sm font-bold text-gray-800 dark:text-white/90 flex items-center gap-1"><i
                                class="fa-solid fa-stopwatch text-indigo-500"></i> Personal Best Time</h4>
                        <div class="w-full sm:w-60">
                            <select id="pbt_filter_selector"
                                class="w-full text-xs rounded-md border-gray-300 shadow-sm text-gray-900 font-semibold bg-white py-1 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
            ...
            <div id="freetext-container" class="hidden mt-4 mb-6">
                <div
                    ...
                    <div
                        ...
                        <div
                            class="p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-100/50 dark:bg-gray-800/80">
                            <h4
                    ...
                    <div class="flex-1 flex flex-col min-w-0 bg-white dark:bg-gray-900">
                        <div id="freetext-detail-panel" class="flex-1 overflow-y-auto p-6 no-scrollbar">
                            <div id="freetext-detail-empty"
                                ...
                                <i
                                    class="fa-regular fa-hand-pointer text-4xl mb-3 text-gray-300 dark:text-gray-600"></i>
                                <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Pilih bulan di samping
                                    kiri</p>
                                <p class="text-xs text-gray-500 dark:text-gray-500 mt-1">Detail catatan perkembangan
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6 pt-6 border-t border-gray-200 dark:border-gray-800">
                <div
                    ...
                    <h4
                        class="text-xs font-bold text-brand-700 dark:text-brand-400 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                        <i class="fa-solid fa-comment-dots"></i> Catatan Terakhir Pelatih
                    </h4>
                    <p id="latest-note" class="text-sm text-gray-700 dark:text-gray-300 italic">"Tidak ada catatan
                        pada evaluasi terakhir."</p>
                    <div id="latest-note-date" class="text-[10px] text-gray-500 dark:text-gray-400 mt-2 font-medium">
                        Diinput pada: -</div>
                </div>

                <div
                    class="bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl p-5 flex flex-col justify-center">
                    <h4
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function destroyAllCharts() {
                ...
                if (prestasiContainer) {
                    prestasiContainer.classList.add('hidden');
                    prestasiContainer.style.display = 'none';
                }
            }
            ...
            function renderChartsForYear(year) {
                ...
                if (isPrestasi) {
                    ...
                    const kondisiFisikEl = document.getElementById('radarChart').closest(
                        '.bg-gray-50, .bg-slate-50, .rounded-xl');
                    const sistemEnergiEl = document.getElementById('barChart').closest(
                        '.bg-gray-50, .bg-slate-50, .rounded-xl');
                    ...
                    if (hasSistemEnergi) {
                        ...
                        barChartInst = new Chart(document.getElementById('barChart').getContext('2d'), {
                            ...
                            data: {
                                ...
                                datasets: [{
                                        label: 'Aerobic',
                                        data: barData.Aerobic,
                                        backgroundColor: 'rgba(16, 185, 129, 0.7)'
                                    },
                                    {
                                        label: 'Anaerobic',
                                        data: barData.Anaerobic,
                        ...
                        if (report.metrics && report.metrics['Personal Best Time']) {
                            ...
                            if (Array.isArray(report.metrics['Personal Best Time'])) {
                                ...
                                entries = [{
                                    gaya: 'Gaya Bebas',
                                    jarak: '50m',
                                    test_per_bulan: report.metrics['Personal Best Time'][
                                        'Test per Bulan'
                                    ] || '',
                    ...
                    function updateLineChartPBT() {
                        ...
                        filteredReports.forEach(report => {
                            ...
                            if (report.metrics && report.metrics['Personal Best Time']) {
                                ...
                                if (Array.isArray(report.metrics['Personal Best Time'])) {
                                    ...
                                    entries = [{
                                        ...
                                        pbt_event: report.metrics['Personal Best Time'][
                                            'PBT Event'
                                        ] || ''
                                    }];
                                }
                            ...
                            if (entry && entry.test_per_bulan) {
                                localLabels.push(d.toLocaleDateString('id-ID', {
                                    month: 'short'
                                }));
                                        ...
                                        callbacks: {
                                            label: function(context) {
                                                if (context.dataset.label === 'PBT Event') {
                                                    const rawText = localPbtData.PbtEvent[context
                                                        .dataIndex].raw;
                    ...
                    if (freetextContainer) {
                        ...
                        function showMonthDetail(report, btnEl) {
                            monthList.querySelectorAll('button').forEach(b => {
                                ...
                                b.classList.add('bg-white', 'text-slate-700', 'hover:bg-indigo-50',
                                    'dark:bg-gray-800', 'dark:text-white/90', 'dark:hover:bg-gray-700');
                            ...
                            if (report.metrics) {
                                for (const [category, items] of Object.entries(report.metrics)) {
                                    ...
                                    for (const [key, val] of Object.entries(items)) {
                                        ...
                                        if (val === 'Sangat Mahir' || val === 'Lulus Tahap Ini' || val ===
                                            'Sudah Lancar') badgeColor = 'bg-green-100 text-green-700';
                                        else if (val === 'Berkembang Baik' || val === 'Mulai Bisa') badgeColor =
                                            'bg-blue-100 text-blue-700';
                                        else if (val === 'Mulai Terlihat') badgeColor =
                                            'bg-amber-100 text-amber-700';
                                        else if (val === 'Belum Berkembang' || val === 'Belum Bisa' || val ===
                                            'Belum Memulai') badgeColor = 'bg-red-100 text-red-700';
                        ...
                        sortedReports.forEach((report, idx) => {
                            ...
                            btn.type = 'button';
                            btn.className =
                                'w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-left text-sm font-semibold transition-all duration-150 bg-white text-slate-700 hover:bg-indigo-50 dark:bg-gray-800 dark:text-white/90 dark:hover:bg-gray-700 border border-transparent';
                            btn.innerHTML = `
                ...
                // Update info murid
                document.getElementById('student-coach').textContent = currentStudent.coach ? currentStudent.coach
                    .name : 'Belum Ditugaskan';
                document.getElementById('student-location').textContent = currentStudent.coach && currentStudent
                    .coach.location ? currentStudent.coach.location.name : '-';
                document.getElementById('student-quota').textContent = currentStudent.remaining_quota != null ?
                    currentStudent.remaining_quota + ' Sesi' : '-';
            }
