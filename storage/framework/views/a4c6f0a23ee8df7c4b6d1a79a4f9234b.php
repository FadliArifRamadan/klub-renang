                                    ...
                                    <td class="px-6 py-4 font-semibold text-slate-700 dark:text-slate-200">
                                        ...
                                        <?php if($leave->schedule): ?>
                                            <div class="inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded bg-[#D3AF37]/15 text-[#D3AF37] border border-[#D3AF37]/30 text-[11px] font-semibold">
                                                <i class="fa-solid fa-clock text-[10px]"></i> <?php echo e($leave->schedule->time_range); ?> — <?php echo e($leave->schedule->swimmingClass->name ?? ''); ?> (<?php echo e($leave->schedule->location->name ?? ''); ?>)
                                            </div>
                                        <?php else: ?>
...
<?php endif; ?>
                                                            <select id="substitute_coach_id-<?php echo e($leave->id); ?>" name="substitute_coach_id" 
                                                                ...
                                                                <?php if(isset($leave->eligible_substitutes) && $leave->eligible_substitutes->isNotEmpty()): ?>
                                                                    <optgroup label="Pelatih Mengajar <?php echo e($leave->target_class_name ?? ''); ?> (Hari <?php echo e($leave->target_day_name ?? ''); ?>)">
                                                                        <?php $__currentLoopData = $leave->eligible_substitutes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $recCoach): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                                            <option value="<?php echo e($recCoach->id); ?>"><?php echo e($recCoach->name); ?> (Bertugas Hari <?php echo e($leave->target_day_name ?? ''); ?>)</option>
                                                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                                                    </optgroup>
                                                                <?php endif; ?>
                                                            </select>
                                                            <?php if(!isset($leave->eligible_substitutes) || $leave->eligible_substitutes->isEmpty()): ?>
                                                                <div class="mt-3.5 p-3.5 bg-amber-500/10 border border-amber-500/30 rounded-xl text-amber-300 text-xs flex items-start gap-3 shadow-sm">
                                                                    ...
                                                                    <div class="leading-relaxed">
                                                                        ...
                                                                        <p class="text-amber-200/90">
                                                                            Tidak ada pelatih lain yang mengajar <strong class="text-white font-semibold"><?php echo e($leave->target_class_name ?? ''); ?></strong> pada hari <strong class="text-white font-semibold"><?php echo e($leave->target_day_name ?? ''); ?></strong>.
                                                                        </p>
