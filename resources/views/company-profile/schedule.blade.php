@extends('layouts.company-profile', ['title' => 'Jadwal Latihan - Black Diamond Swimming Club'])
...
@section('content')
    ...
    <section class="relative pt-32 pb-20 md:pt-44 md:pb-32 bg-[#0B0F17] text-white overflow-hidden border-b border-[#D3AF37]/30">
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
            ...
            <h1 class="text-4xl sm:text-5xl md:text-6xl font-extrabold leading-tight tracking-tight mb-6 text-white">
                Jadwal <span class="text-[#D3AF37]">Latihan Renang</span>
            </h1>
    ...
    <section class="py-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center max-w-2xl mx-auto mb-16 space-y-4">
            <span class="text-xs font-extrabold text-[#D3AF37] uppercase tracking-widest block">JADWAL MINGGUAN</span>
                                            ...
                                            @foreach($sortedSchedules as $sched)
                                                <div class="flex items-center justify-between p-4 bg-slate-55/40 hover:bg-slate-50 rounded-2xl border border-slate-100 transition-colors">
                                                    ...
                                                    <div class="text-right">
                                                        <span class="inline-block text-[10px] bg-amber-50 border border-amber-100 text-amber-700 font-bold px-2 py-0.5 rounded-full mb-1">
                                        ...
                                        @if($sortedSchedules->isNotEmpty())
                                            @foreach($sortedSchedules as $sched)
                                                <div class="flex items-center justify-between p-4 bg-slate-55/40 hover:bg-slate-50 rounded-2xl border border-slate-100 transition-colors">
                                                    ...
                                                    <div class="text-right">
                                                        <span class="inline-block text-[10px] {{ $sched->session_type === 'dryland' ? 'bg-orange-55 border border-orange-100 text-orange-700' : 'bg-amber-55 border border-amber-100 text-amber-700' }} font-bold px-2 py-0.5 rounded-full mb-1">
                                                            {{ $sched->session_type === 'dryland' ? 'Dryland' : 'Berenang' }}
                                                        </span>
                                                        <p class="text-[10px] text-slate-450 font-bold flex items-center gap-1 justify-end">
                                                            <i class="fa-solid fa-location-dot text-[#D3AF37]"></i> {{ $sched->location->name ?? 'Kolam Renang' }}
        ...
        <!-- Note Box -->
        <div class="bg-[#101828] border border-[#D3AF37]/30 rounded-2xl p-6 flex items-start gap-4 text-white shadow-md">
            <div class="w-10 h-10 rounded-xl bg-[#D3AF37]/15 text-[#D3AF37] border border-[#D3AF37]/30 flex items-center justify-center shrink-0 text-sm">
                <i class="fa-solid fa-circle-info"></i>
            </div>
            <div>
                <h4 class="font-extrabold text-[#D3AF37] mb-1">Informasi Penting</h4>
        ...
        <section class="py-20 bg-slate-100 border-y border-slate-200 mb-16">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="text-center max-w-2xl mx-auto mb-16 space-y-4 mt-8">
                    <span class="text-xs font-extrabold text-[#D3AF37] uppercase tracking-widest block">LOKASI
                        LATIHAN</span>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                    @foreach ($locations as $location)
                        <div
                            ...
                            <div
                                class="w-10 h-10 rounded-xl bg-[#D3AF37]/15 text-[#D3AF37] border border-[#D3AF37]/30 flex items-center justify-center shrink-0">
                                <i class="fa-solid fa-water text-sm"></i>
                            </div>
                            <div>
                                <h3 class="font-extrabold text-slate-800 text-sm mb-0.5">{{ $location->name }}</h3>
                                <p class="text-xs text-slate-400 leading-relaxed">{{ $location->address }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
    ...
    <section class="py-20 bg-gradient-to-r from-[#0B0F17] via-[#1E1A0E] to-[#D3AF37] text-white text-center mb-16 shadow-2xl border-y border-[#D3AF37]/40 relative overflow-hidden">
        ...
        <div class="max-w-xl mx-auto px-4 py-6 sm:px-6 lg:px-8 relative z-10">
            <h2 class="text-2xl sm:text-3xl font-extrabold text-[#D3AF37] mb-3 tracking-tight">Siap Memulai Latihan?</h2>
            <p class="text-slate-200 text-sm mb-8 leading-relaxed font-medium">Daftar sekarang dan tim kami akan menghubungi Anda untuk konfirmasi
