<x-app-layout title="Admin - Riwayat Absensi">
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Riwayat Absensi (Semua Coach)') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 border border-gray-100 dark:bg-boxdark dark:border-strokedark">
                ...
                <div class="flex flex-col md:flex-row md:justify-between md:items-center mb-6 gap-4">
                    ...
                    {{-- Search Form --}}
                    <form action="{{ route('admin.attendances.index') }}" method="GET" class="flex gap-2">
                        ...
                        @if(request('search'))
                            <a href="{{ route('admin.attendances.index') }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 rounded-lg border border-gray-300 hover:bg-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-300 dark:bg-meta-4 dark:border-strokedark dark:text-white dark:hover:bg-gray-700 transition-colors">
                                Reset
                            </a>
                        @endif
                    </form>
