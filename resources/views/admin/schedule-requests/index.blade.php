            ...
            <div class="bg-white dark:bg-boxdark p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-strokedark">
                ...
                <div class="flex border-b border-gray-200 dark:border-strokedark mb-6 overflow-x-auto">
                    ...
                    <a href="{{ route('admin.schedule-requests.index', ['status' => 'approved']) }}" 
                       class="py-3 px-5 border-b-2 font-semibold text-sm whitespace-nowrap transition flex items-center gap-2 {{ $status === 'approved' ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:text-gray-200 hover:border-gray-300' }}">
                    ...
                    <a href="{{ route('admin.schedule-requests.index', ['status' => 'rejected']) }}" 
                       class="py-3 px-5 border-b-2 font-semibold text-sm whitespace-nowrap transition flex items-center gap-2 {{ $status === 'rejected' ? 'border-rose-600 text-rose-600' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:text-gray-200 hover:border-gray-300' }}">
                        ...
                        <tbody>
                            @forelse($requests as $index => $req)
                                <tr class="bg-white dark:bg-boxdark border-b border-gray-100 dark:border-strokedark hover:bg-gray-50 dark:bg-meta-4/50 transition-colors">
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="font-semibold text-gray-800 dark:text-gray-100">{{ $req->user->name }}</div>
                                        ...
                                        @if($req->status === 'pending')
                                            ...
                                            <x-modal name="reject-request-{{ $req->id }}" maxWidth="md" focusable>
                                                <form method="POST" action="{{ route('admin.schedule-requests.reject', $req->id) }}" class="p-6 text-left">
                                                    ...
                                                    <div class="mb-4">
                                                        <label for="rejection_reason-{{ $req->id }}" class="text-xs font-bold text-gray-700 dark:text-gray-200 block mb-1">
