    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6" x-data="registrationForm()"
                ...
                <form method="POST" action="{{ route('parent.students.store') }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    {{-- Nomor HP Orang Tua (Readonly dari Profil Akun Parent) --}}
                    <div class="mt-4">
                        <x-input-label for="parent_phone" value="Nomor HP / WhatsApp Orang Tua" />
                        <x-text-input id="parent_phone" class="block mt-1 w-full bg-slate-100 dark:bg-slate-800/80 text-slate-500 dark:text-slate-400 cursor-not-allowed" type="text" value="{{ Auth::user()->phone }}" readonly disabled />
                        <span class="text-[11px] text-slate-400 mt-1 block"><i class="fa-solid fa-circle-info mr-1 text-[#D3AF37]"></i>Otomatis diambil dari nomor WhatsApp profil akun Anda.</span>
                    </div>
                    ...
                    <div class="mt-5" x-show="selectedClassId" x-transition>
                        ...
                        <select id="package_id" name="package_id"
                            ...
                            <template x-for="pkg in filteredPackages" :key="pkg.id">
                                <option :value="pkg.id"
                                    x-text="pkg.name + ' — ' + (selectedLocationId ? formatPrice(getPackagePrice(pkg)) : '(Harga menyesuaikan kolam)') + ' (' + pkg.sessions + 'x Pertemuan)'">
                                </option>
                            </template>
                        </select>
                    ...
                    <div class="mt-6" x-show="selectedPackageId" x-transition>
                        <div
                            class="bg-[#101828] dark:bg-[#101828] border border-[#D3AF37]/30 rounded-2xl p-5 shadow-md">
                            ...
                            <div class="space-y-2.5 text-sm">
                                ...
                                <div class="flex justify-between items-center text-slate-300" x-show="showRegistrationFee">
                                    <span class="font-medium text-xs text-slate-300">Biaya Pendaftaran</span>
                                ...
                                <div class="flex justify-between items-center text-base pt-1">
                                    ...
                                    <span class="font-black text-[#D3AF37] text-xl tracking-tight"
                    ...
                    <div class="flex flex-col items-end mt-8 border-t pt-4">
                        <p class="text-xs text-red-500 mb-2 font-medium"
                            x-show="selectedPackageId && !isPrestasi && selectedScheduleIds.length === 0">
