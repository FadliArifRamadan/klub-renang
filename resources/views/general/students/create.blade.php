            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('general.students.store') }}">
                    ...
                    <div class="mt-4">
                        ...
                        <select id="package_id" name="package_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                            <option value="" disabled selected>-- Pilih Paket Latihan --</option>
                            @foreach ($packages as $package)
                                <option value="{{ $package->id }}" {{ old('package_id') == $package->id ? 'selected' : '' }}>
                                    {{ $package->name }} — Rp {{ number_format($package->price, 0, ',', '.') }} ({{ $package->sessions }}x Pertemuan)
                                </option>
                            @endforeach
                        </select>
