...
{
    /**
     * Menampilkan daftar tagihan di halaman General
     */
    public function index()
    {
        // General user hanya mendaftarkan dirinya sendiri (1 student per user)
    ...
    /**
     * Proses "Konfirmasi Bayar" oleh General user
     */
    public function checkout(Request $request, int $student_id)
    {
        // Pastikan student ini memang milik user yang sedang login
        $student = Student::where('id', $student_id)
            ->where('user_id', Auth::id())
            ->with(['package.locationPrices'])
            ->firstOrFail();
